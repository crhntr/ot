...
func (op Retain) baseLength() int   { return int(op) }
func (op Retain) targetLength() int { return int(op) }
...
func (op Delete) baseLength() int   { return -int(op) }
func (op Delete) targetLength() int { return 0 }
...
func (op Insert) baseLength() int   { return 0 }
func (op Insert) targetLength() int { return len(op) }
...
func Apply(str string, ops ...Applier) (string, error) {
	...
	if len(result) != baseLength(ops) {
		return "", errors.New("could not apply operations to string")
	}
	for _, op := range ops {
...
func Transform(opListA, opListB []Applier) ([]Applier, []Applier, error) {
	...
	for {
		...
		if op, ok := opA.(Insert); ok {
			opAPrime = append(opAPrime, op)
			opBPrime = append(opBPrime, Retain(len([]rune(op))))
			opA = opListA[aIndex]
			aIndex++
			continue
		}
		if op, ok := opB.(Insert); ok {
			opAPrime = append(opAPrime, Retain(len([]rune(op))))
			opBPrime = append(opBPrime, opB)
			opB = opListB[bIndex]
			bIndex++
			continue
		}
